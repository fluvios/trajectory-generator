...
public class TrajectoryIterator implements MultiDataSetIterator {
	 ...
	 * synchronous across all the macrobatches.
	 *
	 * Macrobatch is a group of minibatches. The iterator is modified so that it
	 * reports the end of data when it exhausts a macrobatch. Then it advances
	 * (manually) to the next macrobatch.
	 */
	...
	public TrajectoryIterator(List<List<Double>> corpus, int batchSize, int batchesPerMacrobatch, int dictSize,
			int rowSize) {
		this.corpus = corpus;
	...
	public MultiDataSet next(int num) {
		...
		for (int j = 0; j < currentBatchSize; j++) {
			...
			// replace the entire row in "input" using NDArrayIndex, it's faster
			// than putScalar(); input is NOT made of one-hot vectors
			// because of the embedding layer that accepts token indexes
			// directly
			input.put(
			...
			// prediction (output) and decode ARE one-hots though, I couldn't
			// add an embedding layer on top of the decoder and I'm not sure
			...
			for (Double pred : rowPred) {
				...
				if (predIdx < rowPred.size() - 1) { // put the same vals to
													// decode with +1 offset
	...
	public boolean resetSupported() {
		// we don't want this iterator to be reset on each macrobatch
		// pseudo-epoch
		return false;
	}
