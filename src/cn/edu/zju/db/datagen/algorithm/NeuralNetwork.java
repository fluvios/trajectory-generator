...
public class NeuralNetwork {
    ...
    private final String CHARS = "-\\/_&" + TrajectoryProcessor.SPECIALS;
    ...
    private void run(String[] args, JTextArea txtArea) throws IOException {
        ...
        if (networkFile.exists()) {
            ...
            txtConsoleArea.append("Loading the existing network...");
            ...
            txtConsoleArea.append("Creating a new network...");
            createComputationGraph();
        }
        System.out.println("Number of parameters: " + net.numParams());
        txtConsoleArea.append("Start training ...");
    ...
    private void train(File networkFile, int offset) throws IOException {
        ...
        TrajectoryIterator logsIterator = new TrajectoryIterator(corpus, MINIBATCH_SIZE, MACROBATCH_SIZE, dict.size(), ROW_SIZE);
        for (int epoch = 1; epoch < 10000; ++epoch) {
            System.out.println("Epoch " + epoch);
            txtConsoleArea.append("Epoch " + epoch);            
            if (epoch == 1) {
    ...
    private void startDialog(Scanner scanner) throws IOException {
        ...
        while (true) {
            ...
            TrajectoryProcessor dialogProcessor = new TrajectoryProcessor(new ByteArrayInputStream(line.getBytes(StandardCharsets.UTF_8)), ROW_SIZE,
                    false) {
                @Override
    ...
    private void saveModel(File networkFile) throws IOException {
        ...
        txtConsoleArea.append("Saving the model...");
    ...
    private void createDictionary() throws IOException, FileNotFoundException {
        ...
        TrajectoryProcessor corpusProcessor = new TrajectoryProcessor(toTempPath(CORPUS_FILENAME), ROW_SIZE, true);
        ...
        corpusProcessor = new TrajectoryProcessor(toTempPath(CORPUS_FILENAME), ROW_SIZE, false) {
            @Override
            protected void processLine(String lastLine) {
                List<String> words = new ArrayList<>();
                tokenizeLine(lastLine, words, true);
